...
void autoSave(const std::vector<Expense>& expenses){
    if(!saveExpensesToFile("expenses.txt", expenses)){
        std::cerr << "Warning: Failed to save expenses.\n";
    }
}
void overallSummary(const std::vector<Expense>& expenses){
    double total = 0.0;
    ...
    std::cout << "Number of entries:" << expenses.size() << "\n";
}
void categorySummary(const std::vector<Expense>& expenses){
...
std::string extractMonthYear(const std::string& date){
    if(date.size()>=10){
        return date.substr(3,7);
    }
    return "unknown";
}
void monthlySummary(const std::vector<Expense>& expenses){
    std::map<std::string, double> totals;

    for(const auto& e: expenses){
        std::string key = extractMonthYear(e.date);
        totals[key] += e.amount;
    }
...
void reportsMenu(const std::vector<Expense>& expenses){
    if(expenses.empty()){
        std::cout << "No expenses available for reports.\n";
        do{
            std::cout<<"\n--- Reports Menu ---\n";
            std::cout<<"1. Overall Summary\n";
            std::cout<<"2. Category-wise Summary\n";
            std::cout<<"3. Monthly Summary\n";
            std::cout<<"4. Back\n";
